...
var fileUtilities = {
		...
		moveAsync2: function (sourceFullPath,destFolder,onSuccess){
			var url=sourceFullPath;
			var destFile2=destFolder;
			var ft=new FileTransfer();//Crear objeto FileTransfer
		    ft.download(//Copiar (descargar) el fichero indicado por URL en destFile
				url,
				destFile2,
...
var pageEntzunezkoa={
		...
		createExam: function(i,exams,level){
			...
			var headerDiv=
				...
				'</div>';
			/*
			var statementsDivs='<audio src="'exams[i].audioUrl+'.mp3" controls>'+
								...
								'</audio>';
			*/
			var filefull=exams[i].audioUrl+".mp3";
			var filefull2="file:///android_asset/www/";
			var filefull3='cdvfile:///sdcard/Euskhazi/voice/'+exams[i].audioUrl+'.mp3';
			var filefull4='file:///android_asset/www/'+exams[i].audioUrl+'.mp3';
			
			fileUtilities.moveAsync2(
					filefull4,filefull3,//Mover el fichero de grabación creado "/sdcard/tmprecording.3gp", a la carpeta fileFolder, con nombre fileName
	        	function() {//función successCallback: si el fichero se ha movido
						//audio.media.release();//Liberar el objeto del atributo media
					mediaFile=new Media(filefull3);
					mediaFile.play();
			...
			//var filefull="file:///"+exams[i].audioUrl+".3gp";
			
		
			
			//var statementsDivs='<audio  controls="controls" src="'+filefull3+'"></audio>';
			//var statementsDivs='<audio  controls="controls" src="'+filefull2+exams[i].audioUrl+'.mp3"></audio>';
			/*
			var statementsDivs='<audio controls="controls">'+
			'<source src='+filefull3+'" type="audio/mpeg"/>'+
			'</audio>';
			statementsDivs+='<audio controls="controls">'+
			'<source src='+filefull4+'" type="audio/mpeg"/>'+
			'</audio>';
